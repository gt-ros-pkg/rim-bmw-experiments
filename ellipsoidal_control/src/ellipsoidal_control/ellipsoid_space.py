...
from controller_base import min_jerk_traj
...
class EllipsoidSpace(object):
    ...
    def create_ellipsoidal_path(self, start_ell_pos, start_ell_quat,
        ...
        _, start_ell_rot = PoseConv.to_pos_rot(([0.]*3,start_ell_quat))
        _, end_ell_rot = PoseConv.to_pos_rot(([0.]*3,end_ell_quat))
        rpy = trans.euler_from_matrix(start_ell_rot.T * end_ell_rot) # get roll, pitch, yaw of angle diff
        start_ell_pos = np.array(start_ell_pos)
        end_ell_pos = np.array(end_ell_pos)
