...
int main(int argc, char** argv)
{
  ...
  cv::Point3f box_max = cv::Point3f(1.9, 1.0, 3.0);//(0.8, 0.05, 1.9);

  cv::Point3f box_min2 = cv::Point3f(0.5, -0.5, 3.0);//(1.24, 0.55, 3.06);
  cv::Point3f box_max2 = cv::Point3f(1.9, 1.0, 5.0);//(0.8, 0.05, 1.9);
  ...
  while(ros::ok()){
	...
	continue;
      ...
      //foreMask = cv::Scalar(255);
      // filter out the 'robot box'
      box_filter(cloud, box_min, box_max, box_mask);
      box_filter(cloud, box_min2, box_max2, box_mask2);
      cv::bitwise_and(box_mask, box_mask2, box_mask);
      ...
      cv::bitwise_and(foreMask, box_mask_inliers, foreMask);
      //foreMask.copyTo(foreMask, box_mask);
      //imshow("fore", foreMask);

      //debug
      // depth_im.copyTo(dep_show, box_mask);
      // cv::normalize(dep_show, dep_show, 0.0, 1.0, cv::NORM_MINMAX);
		      ...
		      geometry_msgs::PoseStamped vel, string hum_frame)
{
  ...
  if(mark_arr.markers.size()>0){
    ...
    //make robot origin
      //translate markers
    double trans_x = -mark_arr.markers[robo_marker_id].pose.position.x;
    double trans_y = -mark_arr.markers[robo_marker_id].pose.position.y;
    cout << "Segfault here?" << endl;
    for(int i=0; i<mark_arr.markers.size(); i++)
      {
	mark_arr.markers[i].pose.position.x += trans_x;
	mark_arr.markers[i].pose.position.y += trans_y;
      }
